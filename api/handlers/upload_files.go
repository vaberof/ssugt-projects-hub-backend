...
func UploadFilesHandler(logs *logs.Logs, fileService fileservice.Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		log := logs.WithName("upload-files-handler")
		...
		if len(toSave) == 0 {
			log.Error("Нет валидных файлов для сохранения")
			xhttp.BadRequest(w)
			return
		}

		if err := fileService.Save(ctx, toSave); err != nil {
			log.Error("Ошибка при сохранении файлов:", err)
			xhttp.BadRequest(w)
			return
		}

		response := uploadFilesResponse{Message: "Successfully uploaded files"}
